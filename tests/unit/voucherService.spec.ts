...
describe("Voucher Service unit test", () => {
	...
	it("applyVoucher function should return an error if voucher does not exist", async () => {
		...
		await expect(
			voucherService.applyVoucher(code, MIN_VALUE_FOR_DISCOUNT)
	...
	it("applyVoucher function should not apply voucher if voucher already is used", async () => {
		const invalidVoucher = { ...voucher, used: true };

		jest
			.spyOn(voucherRepository, "getVoucherByCode")
			.mockResolvedValue(invalidVoucher);
	...
	it("applyVoucher function should use voucher", async () => {
		jest
			.spyOn(voucherRepository, "getVoucherByCode")
			.mockResolvedValue(voucher);
